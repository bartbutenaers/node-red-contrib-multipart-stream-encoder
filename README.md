...
In some cases we want very fast a lot of data.  For example we want to get a number of images from our IP camera, to make sure we have a fluent video.  *That is not possible with the above mechanism*, since we would have to wait all the time:
1. Ask image 1
1. Wait for image 1
1. Ask image 2
1. Wait for image 2
...
This just takes too long, so we cannot get a fluent video.  One could workaround this issue by sending http requests in parallel (i.e. simultaneously), but this is a lot of overhead and some IP camera's start choking in the overflow of http requests.

In those latter cases it is much better to use http streaming.  We send a ***single*** http request to our IP camera, and then the camera answers with an (in)finite stream of images.  All IP cameras provide separate URL's for both request-response and streaming.
...
For a finite stream (e.g. playing recorded video footage), it might be useful to send a message with `msg.stop` = true to the encoder node.  In that case, the encoder will stop the http response (which is specified in the `msg.res` field).  Otherwise the client keeps waiting for data, while the stream has already ended (since all data has been streamed already).  E.g. in Chrome the tabsheet will have contain a spinning wheel as long as the response is not yet complete:
...
Watching the images from an IP camera is an example of an infinite stream (of camera images).  **All clients** hook in to the existing stream, and they receive the data ***FROM THAT MOMENT ON***.  Indeed for e.g. a camera stream, you only need to see the images from now on (not the old ones).  he flow can pass data (e.g. images) to the encoder node, even when no clients are listening: the data will simply be ignored.
...
However in some cases not all clients need to get the same stream: that is what the **single client** option is designed for.  E.g. when camera images have been stored, EACH client wants to see the ***ENTIRE STREAM FROM THE BEGINNING***.  Now for each http request, some nodes should be triggered to capture data (and send that data to the encoder node).  It is important that *all messages should contain the response object* in the `msg.res` field, so the encoder node knows which data needs to be send to which client!
...
The easiest way to go is to push (base64 encoded) images directly in a template node:
...
Let's rewrite the template node: now the <img> tag gets all the data by itself from our Node-Red flow, by the 'src' that is referring to our http-in node:
```html
